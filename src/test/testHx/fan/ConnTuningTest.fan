...
//////////////////////////////////////////////////////////////////////////
  ...
  Void testRoster()
  {
    ...
    ext := (ConnExt)addExt("hx.haystack")
    ...
    verifyEq(ext.tuning.id.toStr, "haystack-default")
    verifyEq(ext.tuning.staleTime, 5min)
    verifySame(ext.tuning, ext.conn(c.id).tuning)
    verifySame(ext.tuning, ext.point(pt.id).tuning)
    ...
    verifyEq(ext.tuning.staleTime, 1sec)
    verifySame(ext.tuning, ext.conn(c.id).tuning)
    verifySame(ext.tuning, ext.point(pt.id).tuning)
    verifyTuning(fw, ext, pt, t1, 1sec)

    // add tuning for conn
    commit(c, ["connTuningRef":t2.id])
    sync(c)
    verifyEq(ext.tuning.id, t1.id)
    verifyEq(ext.conn(c.id).tuning.id, t2.id)
    verifyEq(ext.point(pt.id).tuning.id, t2.id)
    verifyNotSame(ext.tuning, ext.conn(c.id).tuning)
    verifySame(ext.conn(c.id).tuning, ext.point(pt.id).tuning)
    verifyTuning(fw, ext, pt, t2, 2sec)
    ...
    verifyEq(ext.tuning.id, t1.id)
    verifyEq(ext.conn(c.id).tuning.id, t2.id)
    verifyEq(ext.point(pt.id).tuning.id, t3.id)
    verifyNotSame(ext.tuning, ext.conn(c.id).tuning)
    verifyNotSame(ext.conn(c.id).tuning, ext.point(pt.id).tuning)
    ...
    sync(c)
    verifyEq(ext.point(pt.id).tuning.id, t5id)
    verifyEq(ext.point(pt.id).tuning.staleTime, 5min)
    verifyDictEq(ext.point(pt.id).tuning.rec, Etc.dict1("id", t5id))

    // now fill in t5
    t5 := addRec(["id":t5id, "dis":"T-5", "connTuning":m, "staleTime":n(123, "sec")])
    sync(c)
    verifyEq(fw.tunings.get(t5id).staleTime, 123sec)
    verifyEq(ext.point(pt.id).tuning.id, t5.id)
    verifyEq(ext.point(pt.id).tuning.staleTime, 123sec)
    verifySame(ext.point(pt.id).tuning.rec, t5)
  }
