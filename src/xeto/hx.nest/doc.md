...
device access project by creating a [NestConn.nestConn] rec (the values in `<>` are obtained
...
# Supported Points
The nest connector uses the [NestPoint.nestCur] tag to get the current value of a particular
...
current room temperature by setting [NestPoint.nestCur] to `THERM-123:Temperature.ambientTemperatureCelsius`
...
[NestPoint.nestWrite] tag is configured for those points. If you use [connLearn()], then
