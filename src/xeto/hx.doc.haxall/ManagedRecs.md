...
# Overview
The runtime configuration is stored as normal records in the [Folio]
...
There are five different record types represented by the `rt` tag enum:
  ...
  - [func](#func-companion-recs): used to add a function in the companion lib
  - [spec](#spec-companion-recs): used to add a Xeto spec in the companion lib
  - [instance](#instance-companion-recs): used to add a Xeto instance in the companion lib

Each of these managed record types is discussed further below.

# Meta
Every runtime has a metadata dict you can access in Fantom via [fan.hx::Runtime.meta]
...
The following are the Fantom APIs:
  - [fan.hx::RuntimeLibs]: add/remove libs
  - [fan.hx::Ext.settings]: read settings
...
The following Axon functions are used to work with the companion lib managed recs:
  - [companionReadByName()]: read by name
