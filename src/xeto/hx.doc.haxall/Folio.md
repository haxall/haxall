...
data.  But it also provides first class support for the [Haysatck ontology](ph.doc::Ontology)
so that you can model your data in a standardized and reusable manner.

Folio organizes data into a three level hierarchy:

  - [Projects](#projects): or "projs" are the top level unit of organization
  ...
  - [Records](#records): or "recs" are the basic unit of data modeling.  Records
...
All records are a Haystack [dict](ph.doc::Kinds#dict) - a map of
...
dicts according to the standardized [ontology](ph.doc::Ontology).
...
any of the Haystack [kinds](ph.doc::Kinds):
one of the following scalar types:
  - [Marker](fan.xeto::Marker): indicates the tag is used solely to mark
    the record; markers are typically used to assign the record into a "type"
  - [Ref](fan.xeto::Ref): identifier for Folio records
  - [Bool](fan.sys::Bool): true of false
  - [Number](fan.haystack::Number): 64-bit floating point number with optional [Units]
  - [Str](fan.sys::Str): unicode string
  - [Uri](fan.sys::Uri): universal resource identifier per RFC 3986
  - [Date](fan.sys::Date): standard Fantom date class
  - [Time](fan.sys::Time): standard Fantom hour of day class
  - [DateTime](fan.sys::DateTime): standard Fantom timestamp with timezone
  - [Bin](fan.haystack::Bin): binary streamed data stored on file system
  - [Coord](fan.haystack::Coord): geographic coordinate in latitude/longitude
  - [XStr](fan.haystack::XStr): extended type string
  - [List](fan.sys::List): linear sequence of zero or more items
  - [Dict](fan.xeto::Dict): nested dict of name/value pairs
  - [Grid](fan.haystack::Grid): nested two dimensional grid
...
[Programmatic names](ph.doc::Kinds#names) use `camelCaseNaming` as
follows:
...
database using [Folio.passwords](fan.folio::Folio.passwords) method.
...
and history data.
  ...
  - Fantom API: [fan.folio::Folio.backup]
...
following tags may be configured on the projMeta rec:
  ...
  - [steadyState](Runtime#steady-state): delay for steady state transition
