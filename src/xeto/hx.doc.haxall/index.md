# Getting Started

- [Setup](Setup.md): Getting Haxall installed and running
- [Build](Build.md): Build from source

# Architecture

- [Architecture](Architecture.md): Overview of the software architecture
- [Runtime](Runtime.md): Runtime overview
- [Folio](Folio.md): Tag database
- [Axon](Axon.md): Axon scripting language
- [Namespace](Namespace.md): Xeto lib namespace
- [Exts](Exts.md): Extensions
- [ManagedRecs](ManagedRecs.md): Managed recs for meta, libs, and companion lib
- [Observables](Observables.md): Observable subscription subsystem
- [Watches](Watches.md): Subscription to record changes
- [Crypto](Crypto.md): Help on managing keys and trusted certificates

# Axon

- [AxonLang](AxonLang.md): Features of Axon programming language
- [Streams](Streams.md): Asynchronous streams
- [AxonUsage](AxonUsage.md): Example snippets of Axon code
- [Units](Units.md): Units of measurement

# Connectors

- [Conns](Conns.md): Overview to using connectors
- [ConnTuning](ConnTuning.md): Guide to tuning connector performance
- [CustomConns](CustomConns.md): Creating your own custom connector

# Formats and Protocols

- [Xml](Xml.md): Extensible markup language

# Appendix

- [AxonGrammar](AxonGrammar.md): Formal BNF syntax grammar
- [ChangeLog](ChangeLog.md): List of changes in each build

