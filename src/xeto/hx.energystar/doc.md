...
An EnergyStar connector supports the following tags:
  ...
  - [EnergyStarConn.energyStarConn]: required marker tag
  - [hx::User.username]: account username
  - `password`: must have password stored in [password db](hx.doc.haxall::Folio#passwords)
    for connector's record id
  - [EnergyStarConn.uri]: optional URI to support switching between the live
...
See [ph::PhEntity.primaryFunction] tag for valid string values.
...
"Electric".  Otherwise the point must define the [EnergyStarPoint.energyStarMeterType]
tag with one of the valid string values.

If the meter is a "waste" meter you must set the [EnergyStarPoint.energyStarMeterType] tag on
