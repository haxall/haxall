...
# Lifecycle
Task records are mapped one-to-one to an [actor](fan.concurrent::Actor) by
...
# Subscriptions
Tasks may subscribe to an [observable](hx.doc.haxall::Observables).  Subscribed
tasks receive observations from the observable as their messages to process.

Tasks subscribe to an [hx.obs::Observable] by applying one of the following marker tags:
  - [obsSchedule](hx.doc.haxall::Observables#schedule): observe scheduled events
  - [obsCommits](hx.doc.haxall::Observables#commits): observe commits to the Folio database
  - [obsWatches](hx.doc.haxall::Observables#watches): observe watch status of records
  - [obsCurVals](hx.doc.haxall::Observables#cur-vals): observe transient commits to curVal and curStatus
  - [obsHisWrites](hx.doc.haxall::Observables#his-writes): observe writes to the historian
  - [obsPointWrites](hx.doc.haxall::Observables#point-writes): observe updates to a writable point
  - [obsArcs](hx.doc.haxall::Observables#arcs): observe when arc documents are created or modified
  - [obsSparks](hx.doc.haxall::Observables#sparks): observe when new sparks are detected
...
All three functions will return a [future](hx.task::doc#futures) which is a
