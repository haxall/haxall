...
of [point](ph.doc::Points) records that model sensors, commands,
...
with the [ph::PhEntity.cur] marker tag.
...
and status:
  ...
  - [ph::PhEntity.curStatus]: ok, stale, down, fault, disabled, unknown,
...
to Folio using [transient diffs](hx.doc.haxall::Folio#transient-diffs).
...
The current value of points are synchronized one of two ways:
  ...
  - the point is added to a [watch](hx.doc.haxall::Watches)
...
Any point rec can be turned into a writable point by adding the [ph::PhEntity.writable]
tag.  Writable points should be [ph::PhEntity.cmd] or [ph::PhEntity.sp] points (never [ph::PhEntity.sensor] points).
...
following tags are transiently updated:
  ...
  - [ph::PhEntity.writeLevel]: number from 1 to 17 indicate the winning priority array level
  - [ph::PhEntity.writeStatus]: status of the connector's ability to write the last
  ...
  - [ph::PhEntity.writeErr]: indicates the error message if writeStatus is error condition

Because the write value might be changing several times a second, the tags
above are managed transiently.  This means they are not written to disk,
nor persisted between server restarts.  Changes to these tags are written
to Folio using [transient diffs](hx.doc.haxall::Folio#transient-diffs).  They
...
Any point may be historized in one of two ways:
  - [conn sync](hx.doc.haxall::Conns#point-history-sync): if the remote system supports its
    own data logging facilities, then the point's connector should implement
    history synchronization
  - [collect](hx.point::doc#his-collect): the point history may be be collected
...
using the [task lib](hx.task::doc).
...
updating the [ph::PhEntity.curVal] tag.

History collection is setup in two ways:
  - [Point.hisCollectCov]: setups up change of value collection
  - [Point.hisCollectInterval]: setups up interval sample collection
...
The following tags may be used to tune local history collection:
  - [Point.hisCollectNA]: used to log NA when status is not "ok"
  - [Point.hisCollectCovRateLimit]: used to throttle rapidly changing data
  - [Point.hisCollectWriteFreq]: used to buffer items in RAM before flushing to Folio
...
the [EnumMeta.enumMeta] marker tag.  Each string tag defines an enum definition.
The name of the tag defines the enum identifier.  The value is a
string formatted as a [Zinc](ph.doc::Zinc) grid with a `name`
and `code` column.
...
these names with [point conversion](hx.point::doc#point-conversion) as follows:
